...
**Decoder inventory:** the complete byte-parsing decoders are Rust — TIFF, EXR, PNG16, HDR, JPEG, JPEG XR, PFM, NetPBM, NPY/NPZ, FITS, NetCDF, DICOM, CZI, ND2 and LIF. Common binary NetPBM, native little-endian float32 NPY, and native-endian PFM additionally have conservative zero-copy TypeScript accelerators; binary P5 runs directly because its conversion is cheaper than a worker round trip, while the larger raw paths use the small worker. Unsupported variants go to Rust. `geotiff.js`, `parse-exr`, `upng` and `pako` remain only for cases Rust does not yet cover, not as general fallbacks. OME-XML parsing ([media/modules/ome-tiff.ts](media/modules/ome-tiff.ts)) stays TypeScript deliberately: the byte-level extraction is already in Rust (`extract_ome_xml`), and what remains maps XML text onto a typed model consumed by dataset-navigation UI, which is the TS side of the rule above.
