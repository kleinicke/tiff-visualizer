...
pub(crate) fn build_histogram(plane: &[f32], step: usize) -> ScalarHistogram {
    ...
    if !min.is_finite() || !max.is_finite() {
        return ScalarHistogram { counts, min: 0.0, max: 0.0, total: 0, non_finite_count: non_finite };
    }
    ...
    if max == min {
        ...
        return ScalarHistogram { counts, min, max, total, non_finite_count: non_finite };
    }
    ...
    while i < plane.len() {
        ...
        if v.is_finite() {
            ...
            if bin >= HISTOGRAM_BINS as i64 { bin = HISTOGRAM_BINS as i64 - 1; }
            if bin < 0 { bin = 0; }
    ...
    ScalarHistogram { counts, min, max, total, non_finite_count: non_finite }
}
...
fn otsu(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    if n == 0 { return -1; }
    let mut sum: f64 = 0.0;
    for i in 0..HISTOGRAM_BINS { sum += (i as f64) * counts[i] as f64; }
    ...
    for t in 0..HISTOGRAM_BINS {
        ...
        if variance > best_variance { best_variance = variance; best = t as i32; }
    }
    best
}
...
fn iso_data(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    for i in 0..HISTOGRAM_BINS {
        if counts[i] > 0 { t = i as i32; break; }
    }
    ...
    while threshold != previous && guard < 1000 {
        ...
        if count_below == 0 || count_above == 0 { break; }
    ...
    if threshold < t { t } else { threshold }
}
...
fn li(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    for i in 0..HISTOGRAM_BINS { mean += (i as f64) * counts[i] as f64; }
    mean /= n as f64;
    ...
    loop {
        ...
        let t_idx = if t < 0.0 { 0i64 } else if t >= HISTOGRAM_BINS as f64 { HISTOGRAM_BINS as i64 - 1 } else { t as i64 };
        let t_idx = t_idx as usize;

        let mut sum_back: f64 = 0.0;
        let mut count_back: i64 = 0;
        for i in 0..=t_idx { sum_back += (i as f64) * counts[i] as f64; count_back += counts[i] as i64; }
        let mut sum_fore: f64 = 0.0;
        let mut count_fore: i64 = 0;
        for i in (t_idx + 1)..HISTOGRAM_BINS { sum_fore += (i as f64) * counts[i] as f64; count_fore += counts[i] as i64; }

        let mean_back = if count_back > 0 { sum_back / count_back as f64 } else { 0.0 };
        let mean_fore = if count_fore > 0 { sum_fore / count_fore as f64 } else { 0.0 };
        let a = if mean_back > 0.0 { mean_back } else { 1e-9 };
        let b = if mean_fore > 0.0 { mean_fore } else { 1e-9 };
        new_threshold = (b - a) / (b.ln() - a.ln());
        if !new_threshold.is_finite() { break; }
        guard += 1;
        if (new_threshold - old_threshold).abs() <= 0.5 || guard >= 1000 { break; }
    }
...
fn triangle(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    let mut peak = 0usize;
    for i in 1..HISTOGRAM_BINS { if counts[i] > counts[peak] { peak = i; } }

    let mut first = 0usize;
    while first < HISTOGRAM_BINS && counts[first] == 0 { first += 1; }
    let mut last = HISTOGRAM_BINS - 1;
    while last > 0 && counts[last] == 0 { last -= 1; }
    if first >= last { return -1; }

    if peak - first < last - peak {
        let mut reversed = [0i32; HISTOGRAM_BINS];
        for i in 0..HISTOGRAM_BINS { reversed[i] = counts[HISTOGRAM_BINS - 1 - i]; }
        let result = triangle_one_sided(&reversed);
        return if result < 0 { result } else { HISTOGRAM_BINS as i32 - 1 - result };
    }
...
fn triangle_one_sided(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    for i in 1..HISTOGRAM_BINS { if counts[i] > counts[peak] { peak = i; } }
    let mut last = HISTOGRAM_BINS - 1;
    while last > peak && counts[last] == 0 { last -= 1; }
    if last <= peak { return peak as i32; }

    let dx = (last - peak) as f64;
    let dy = (counts[last] - counts[peak]) as f64;
    let norm = { let n = dx.hypot(dy); if n == 0.0 { 1.0 } else { n } };
    let mut best = peak;
    let mut best_distance: f64 = -1.0;
    for i in peak..=last {
        let distance = (dy * (i - peak) as f64 - dx * (counts[i] - counts[peak]) as f64).abs() / norm;
...
fn yen(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    for i in 0..HISTOGRAM_BINS { p[i] = counts[i] as f64 / n as f64; }
    ...
    for t in 0..HISTOGRAM_BINS {
        ...
        let b = p1[t] * (1.0 - p1[t]);
        let criterion = (if a > 0.0 { -1.0 * a.ln() } else { 0.0 }) + (if b > 0.0 { 2.0 * b.ln() } else { 0.0 });
        if criterion > best_criterion { best_criterion = criterion; best = t as i32; }
    }
    best
}
...
fn huang(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    if first == last { return first as i32; }
    if first >= HISTOGRAM_BINS { return -1; }
    ...
    for i in 0..HISTOGRAM_BINS {
        ...
        membership_cost[i] = -membership * membership.ln() - (1.0 - membership) * (1.0 - membership).ln();
    }
    ...
    for t in first..=last {
        let mut entropy = 0.0;
        let mean_low = if cumulative[t] > 0.0 { (weighted[t] / cumulative[t]).round() } else { 0.0 };
        for i in first..=t {
        ...
        let mean_high = if high_count > 0.0 { ((weighted[last] - weighted[t]) / high_count).round() } else { 0.0 };
        for i in (t + 1)..=last {
            let idx = ((i as f64 - mean_high).abs()) as usize;
            let idx = idx.min(HISTOGRAM_BINS - 1);
            entropy += membership_cost[idx] * counts[i] as f64;
        }
        if entropy < best_entropy { best_entropy = entropy; best = t; }
    }
    best as i32
}
...
fn max_entropy(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    let mut cumulative = [0f64; HISTOGRAM_BINS];
    cumulative[0] = p[0];
    for i in 1..HISTOGRAM_BINS { cumulative[i] = cumulative[i - 1] + p[i]; }
    ...
    for t in 0..HISTOGRAM_BINS {
        ...
        if p_background <= 0.0 || p_foreground <= 0.0 { continue; }
        let mut background_entropy = 0.0;
        for i in 0..=t {
            if p[i] > 0.0 { background_entropy -= (p[i] / p_background) * (p[i] / p_background).ln(); }
        }
        let mut foreground_entropy = 0.0;
        for i in (t + 1)..HISTOGRAM_BINS {
            if p[i] > 0.0 { foreground_entropy -= (p[i] / p_foreground) * (p[i] / p_foreground).ln(); }
        }
        let entropy = background_entropy + foreground_entropy;
        if entropy > best_entropy { best_entropy = entropy; best = t as i32; }
    }
    best
}
...
fn moments(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    for i in 0..HISTOGRAM_BINS {
        ...
        if cumulative > p0 { return i as i32; }
    }
    -1
}
...
fn percentile(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    for i in 0..HISTOGRAM_BINS {
        ...
        if distance < best_distance { best_distance = distance; best = i as i32; }
    }
    best
}
...
fn shanbhag(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    ...
    for t in 0..HISTOGRAM_BINS {
        ...
        if distance < best_distance { best_distance = distance; best = t as i32; }
    }
    best
}
...
fn smooth_to_bimodal(counts: &[i32; HISTOGRAM_BINS]) -> Option<[f64; HISTOGRAM_BINS]> {
    ...
    for i in 0..HISTOGRAM_BINS { smoothed[i] = counts[i] as f64; }
    for _ in 0..10000 {
        let mut peaks = 0;
        for i in 1..HISTOGRAM_BINS - 1 {
            if smoothed[i - 1] < smoothed[i] && smoothed[i + 1] < smoothed[i] { peaks += 1; }
        }
        if peaks <= 2 {
...
fn minimum(counts: &[i32; HISTOGRAM_BINS]) -> i32 {
    let smoothed = match smooth_to_bimodal(counts) { Some(s) => s, None => return -1 };
    for i in 1..HISTOGRAM_BINS - 1 {
        if smoothed[i - 1] > smoothed[i] && smoothed[i + 1] >= smoothed[i] { return i as i32; }
    }
    -1
}
...
pub(crate) fn compute_stability_curve(
    ...
    for s in 0..samples {
        let bin = ((s as f64 / (samples as f64 - 1.0)) * (HISTOGRAM_BINS as f64 - 1.0)).round() as i32;
        let value = threshold_value_from_bin(histogram, bin);
        let mut inside: u32 = 0;
        for i in 0..small.len() {
            let v = small[i] as f64;
            let on = v.is_finite() && (if dark_background { v >= value } else { v <= value });
        ...
        points.push(StabilityPoint {
            ...
            area_fraction: if mask.is_empty() { 0.0 } else { inside as f64 / mask.len() as f64 },
        });
    }
    ...
    for i in 1..=points.len() {
        let ended = i == points.len() || {
            let diff = (points[i].object_count as i64 - points[run_start].object_count as i64).abs();
        ...
        let usable = midpoint.object_count > 0 && midpoint.area_fraction > 0.0005 && midpoint.area_fraction < 0.95;
        if usable && length > best_length { best_length = length; best_start = run_start; }
        run_start = i;
    }
    ...
    StabilityCurve { points, suggested_bin, plateau_width: best_length as i32 }
}
...
pub(crate) fn downsample(
    ...
    for y in 0..out_height {
        for x in 0..out_width {
            ...
            out[y * out_width + x] = if n > 0 { (sum / n as f64) as f32 } else { f32::NAN };
        }
    }
...
fn count_components(mask: &[u8], width: usize, height: usize) -> u32 {
    ...
    for start in 0..mask.len() {
        ...
        while let Some(index) = stack.pop() {
            let x = index % width;
            let y = index / width;
            if x > 0 && mask[index - 1] != 0 && visited[index - 1] == 0 { visited[index - 1] = 1; stack.push(index - 1); }
            if x + 1 < width && mask[index + 1] != 0 && visited[index + 1] == 0 { visited[index + 1] = 1; stack.push(index + 1); }
            if y > 0 && mask[index - width] != 0 && visited[index - width] == 0 { visited[index - width] = 1; stack.push(index - width); }
            if y + 1 < height && mask[index + width] != 0 && visited[index + width] == 0 { visited[index + width] = 1; stack.push(index + width); }
        }
    }
    components
}
...
pub(crate) fn local_threshold_mask(
    ...
    for &v in input_plane.iter() {
        ...
        if v < source_min { source_min = v; }
        if v > source_max { source_max = v; }
    }
    if !source_min.is_finite() || !source_max.is_finite() { return out; }

    let plane: Vec<f32> = if bright_objects {
        let pivot = source_min + source_max;
        input_plane.iter().map(|&v| {
            let v = v as f64;
            if v.is_finite() { (pivot - v) as f32 } else { f32::NAN }
        }).collect()
    } else {
        input_plane.to_vec()
    };
    ...
    if options.method == "median" {
        return local_median_mask(&plane, width, height, radius as usize, options.offset.unwrap_or(0.0));
    }
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            sum_squares[i] = value * value + sum_squares[up] + sum_squares[left] - sum_squares[up_left];
            counts[i] = (if finite { 1.0 } else { 0.0 }) + counts[up] + counts[left] - counts[up_left];
        }
    }
    ...
    let range = if source_max > source_min { source_max - source_min } else { 1.0 };
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            let threshold = match options.method.as_str() {
                ...
                "phansalkar" => {
                    let normalized = if range > 0.0 { (mean - global_min) / range } else { 0.0 };
...
fn local_median_mask(plane: &[f32], width: usize, height: usize, radius: usize, offset: f64) -> Vec<u8> {
    let mut out = vec![0u8; width.saturating_mul(height)];
    if out.is_empty() { return out; }
    let mut min = f64::INFINITY;
    let mut max = f64::NEG_INFINITY;
    ...
    if !min.is_finite() || max <= min { return out; }
    let bins = 64usize;
        ...
        for x in 0..width {
            ...
            for wy in y0..=y1 {
                for wx in x0..=x1 {
                    ...
                    if !v.is_finite() { continue; }
                    let mut bin = ((v - min) * scale).floor() as i64;
                    if bin >= bins as i64 { bin = bins as i64 - 1; }
                    if bin < 0 { bin = 0; }
                    counts[bin as usize] += 1;
                    n += 1;
                }
            ...
            for b in 0..bins {
                ...
                if cumulative * 2 >= n { median_bin = b; break; }
            }
...
pub(crate) fn local_auto_threshold_mask(
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            if valid[nearest_y * tiles_x + nearest_x] == 0 { continue; }
            ...
            for (cy, cx, w) in corners {
                ...
                if valid[index] == 0 || w <= 0.0 { continue; }
            ...
            let on = if dark_background { value >= threshold } else { value <= threshold };
...
pub(crate) fn global_threshold_mask(plane: &[f32], low: f64, high: f64) -> Vec<u8> {
    ...
    for i in 0..plane.len() {
        let v = plane[i] as f64;
        if v.is_finite() && v >= low && v <= high { out[i] = 1; }
    }
    out
}
...
mod tests {
    ...
    fn bimodal_counts() -> [i32; HISTOGRAM_BINS] {
        ...
        let mut counts = [0i32; HISTOGRAM_BINS];
        for i in 0..50 { counts[i] = 10; }
        for i in 200..HISTOGRAM_BINS { counts[i] = 10; }
        counts
    }
    ...
    fn otsu_splits_bimodal_histogram_in_the_gap() {
        ...
        let bin = otsu(&counts);
        assert!(bin >= 49 && bin < 200, "expected split in the gap, got {bin}");
    }
    ...
    fn degenerate_inputs_do_not_panic() {
        ...
        let opts = LocalThresholdOptions { method: "sauvola".to_string(), radius: 3.0, k: 0.5, r: None, offset: None, dark_background: true };
        assert_eq!(local_threshold_mask(&[], 0, 0, &opts), Vec::<u8>::new());
        let opts2 = LocalAutoThresholdOptions { method: "otsu".to_string(), radius: 8.0, dark_background: true, min_contrast: None };
        assert_eq!(local_auto_threshold_mask(&[], 0, 0, &opts2), Vec::<u8>::new());
    ...
    fn local_threshold_mask_flags_bright_blob_on_dark_background() {
        ...
        assert_eq!(mask[4 * width + 4], 1, "centre of the bright blob must be foreground");
    ...
    fn bin_to_value_and_threshold_value_from_bin_agree_with_the_binning_convention() {
        let histogram = ScalarHistogram { counts: vec![0; HISTOGRAM_BINS], min: 0.0, max: 256.0, total: 0, non_finite_count: 0 };
