...
impl CfaPattern {
    fn new(period: usize, grid: Vec<u8>) -> Self {
        ...
        CfaPattern { period, grid, channels }
    }
...
pub fn pattern_from_name(name: &str) -> CfaPattern {
    match name {
        ...
        "quad_rggb" => CfaPattern::new(4, vec![
            R, R, G, G,
            R, R, G, G,
            G, G, B, B,
            G, G, B, B,
...
pub fn neutralize_cfa_photometric(data: &[u8]) -> Option<Vec<u8>> {
    ...
    let u16_at = |b: &[u8], o: usize| -> u16 {
        if little { u16::from_le_bytes([b[o], b[o + 1]]) } else { u16::from_be_bytes([b[o], b[o + 1]]) }
    };
    let u32_at = |b: &[u8], o: usize| -> u32 {
        let s = [b[o], b[o + 1], b[o + 2], b[o + 3]];
        if little { u32::from_le_bytes(s) } else { u32::from_be_bytes(s) }
    };
    let u64_at = |b: &[u8], o: usize| -> u64 {
        let mut s = [0u8; 8];
        s.copy_from_slice(&b[o..o + 8]);
        if little { u64::from_le_bytes(s) } else { u64::from_be_bytes(s) }
    };
    ...
    for _ in 0..64 {
        ...
        let (count, entry_size, entries_at) = if big {
            if ifd + 8 > data.len() { break; }
            (u64_at(data, ifd) as usize, 20usize, ifd + 8)
        } else {
            if ifd + 2 > data.len() { break; }
        ...
        next = if big { u64_at(data, end) } else { u32_at(data, end) as u64 };
    }
...
impl DemosaicResult {
    ...
    #[wasm_bindgen(getter)]
    pub fn gain_r(&self) -> f32 { self.gain_r }
    #[wasm_bindgen(getter)]
    pub fn gain_g(&self) -> f32 { self.gain_g }
    #[wasm_bindgen(getter)]
    pub fn gain_b(&self) -> f32 { self.gain_b }
...
fn gray_world_gains(
    ...
    let mean = |c: usize| if counts[c] > 0 { sums[c] / counts[c] as f64 } else { 0.0 };
    ...
    let g = |m: f64| if m > 1e-9 { (reference / m) as f32 } else { 1.0 };
    (g(mr), g(mg), g(mb))
}
        ...
        for x in 0..width {
            ...
            for c in 0..nch {
                ...
                for dy in -radius..=radius {
                    ...
                    if sy < 0 || sy >= height as isize { continue; }
                    for dx in -radius..=radius {
                        let sx = x as isize + dx;
                        if sx < 0 || sx >= width as isize { continue; }
                        if pat.at(sx as usize, sy as usize, ox, oy) as usize != c { continue; }
...
fn tap(plane: &[f32], width: usize, height: usize, x: isize, y: isize) -> f32 {
    ...
    let cx = if x < 0 { (-x) as usize } else if x >= width as isize { 2 * width - 2 - x as usize } else { x as usize };
    let cy = if y < 0 { (-y) as usize } else if y >= height as isize { 2 * height - 2 - y as usize } else { y as usize };
...
fn fill_malvar(
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let cross4 = tap(plane, width, height, xi - 1, yi) + tap(plane, width, height, xi + 1, yi)
                + tap(plane, width, height, xi, yi - 1) + tap(plane, width, height, xi, yi + 1);
            let diag4 = tap(plane, width, height, xi - 1, yi - 1) + tap(plane, width, height, xi + 1, yi - 1)
                + tap(plane, width, height, xi - 1, yi + 1) + tap(plane, width, height, xi + 1, yi + 1);
            let far_h = tap(plane, width, height, xi - 2, yi) + tap(plane, width, height, xi + 2, yi);
            let far_v = tap(plane, width, height, xi, yi - 2) + tap(plane, width, height, xi, yi + 2);
...
pub fn demosaic(
    ...
    if w == 0 || h == 0 || data.len() < w * h {
        return Err(JsValue::from_str(&format!(
            "demosaic: plane too small ({} values for {}x{})", data.len(), w, h
        )));
    }
...
mod tests {
    ...
    fn roundtrip_psnr(algorithm: &str, pattern: &str, w: usize, h: usize) -> f64 {
        ...
        let mut res = demosaic(
            &mosaic, w as u32, h as u32, pattern, algorithm,
    ...
    fn malvar_beats_bilinear_on_gradients() {
        ...
        assert!(malvar >= bilinear - 1.0, "malvar {malvar} far below bilinear {bilinear}");
    }
    ...
    fn nearest_is_exact_at_sampled_sites() {
        ...
        let mut res = demosaic(
            &plane, w as u32, h as u32, "rggb", "nearest",
            0, 0, 0.0, 0.0, false, 1.0, 1.0, 1.0,
        ).unwrap();
        let out = res.take_data();
        for y in 0..h {
    ...
    fn every_pattern_produces_full_output() {
        ...
        for name in ["rggb", "bggr", "grbg", "gbrg", "rgbi_4x4", "xtrans", "rccb", "rccc", "rgbw", "quad_rggb"] {
            let pat = pattern_from_name(name);
            let mut res = demosaic(
                &plane, w as u32, h as u32, name, "bilinear",
                0, 0, 0.0, 0.0, false, 1.0, 1.0, 1.0,
            ).unwrap();
            assert_eq!(res.channels() as usize, pat.channels, "{name} channel count");
            let out = res.take_data();
    ...
    fn gray_world_recovers_known_gains() {
        ...
        let res = demosaic(
            &mosaic, w as u32, h as u32, "rggb", "bilinear",
            0, 0, 0.0, 0.0, true, 1.0, 1.0, 1.0,
        ).unwrap();
        assert!((res.gain_r() - 2.0).abs() < 0.01, "gain_r = {}", res.gain_r());
        assert!((res.gain_g() - 1.0).abs() < 0.01, "gain_g = {}", res.gain_g());
        assert!((res.gain_b() - 1.0 / 0.6).abs() < 0.01, "gain_b = {}", res.gain_b());
    }
    ...
    fn black_level_is_subtracted() {
        ...
        let mut res = demosaic(
            &plane, w as u32, h as u32, "rggb", "bilinear",
            0, 0, 256.0, 4351.0, false, 1.0, 1.0, 1.0,
        ).unwrap();
        let out = res.take_data();
        for v in out {
    ...
    fn offset_cycles_through_the_2x2_phases() {
        ...
        let mut a = demosaic(&plane, w as u32, h as u32, "rggb", "bilinear", 1, 0, 0.0, 0.0, false, 1.0, 1.0, 1.0).unwrap();
        let mut b = demosaic(&plane, w as u32, h as u32, "grbg", "bilinear", 0, 0, 0.0, 0.0, false, 1.0, 1.0, 1.0).unwrap();
