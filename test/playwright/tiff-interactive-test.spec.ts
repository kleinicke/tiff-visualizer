...
test.describe('TIFF Interactive Tests', () => {
  ...
  test('should open UINT8 TIFF file and test mouse interactions', async ({ page }) => {
    ...
    // Open the UINT8 TIFF file using VS Code's file opening mechanism
    await page.evaluate(async (imagePath) => {
      console.log('Opening UINT8 file:', imagePath);
      ...
      try {
        ...
        // Wait a moment for the custom editor to activate
    ...
    // Test mouse movement and pixel reading
    const canvasBoundingBox = await canvas.boundingBox();
    if (canvasBoundingBox) {
      // Move mouse to different positions on the canvas
      ...
      console.log('✅ Mouse moved to center of image');
      await page.waitForTimeout(500);
      
      // Move to top-left
      await page.mouse.move(canvasBoundingBox.x + 10, canvasBoundingBox.y + 10);
      console.log('✅ Mouse moved to top-left of image');
      await page.waitForTimeout(500);
      
      // Move to bottom-right
      await page.mouse.move(
        canvasBoundingBox.x + canvasBoundingBox.width - 10,
        canvasBoundingBox.y + canvasBoundingBox.height - 10
      );
      console.log('✅ Mouse moved to bottom-right of image');
      await page.waitForTimeout(500);
    }
  ...
  test('should open Float32 TIFF file and test mouse interactions', async ({ page }) => {
    // Navigate to VS Code Web
    await page.goto('/');
    ...
    // Open the Float32 TIFF file using VS Code's file opening mechanism
    await page.evaluate(async (imagePath) => {
      console.log('Opening Float32 file:', imagePath);
    ...
    // Test mouse movement and pixel reading for float image
    const canvasBoundingBox = await canvas.boundingBox();
    if (canvasBoundingBox) {
      // Move mouse to different positions on the canvas
      const centerX = canvasBoundingBox.x + canvasBoundingBox.width / 2;
      ...
      // Move to center
      await page.mouse.move(centerX, centerY);
      console.log('✅ Mouse moved to center of float image');
      await page.waitForTimeout(500);
      
      // Move to top-left
      await page.mouse.move(canvasBoundingBox.x + 10, canvasBoundingBox.y + 10);
      console.log('✅ Mouse moved to top-left of float image');
      await page.waitForTimeout(500);
      ...
      console.log('✅ Mouse moved to bottom-right of float image');
      await page.waitForTimeout(500);
    }
  ...
  test('should test status bar entries for image information', async ({ page }) => {
    // Navigate to VS Code Web
    await page.goto('/');
    ...
    await page.evaluate(async (imagePath) => {
      console.log('Opening UINT8 file for status bar test:', imagePath);
    ...
    // Look for specific status bar items that should be present
    const statusItems = await page.locator('.statusbar-item');
    const statusCount = await statusItems.count();
    expect(statusCount).toBeGreaterThan(0);
    ...
    // Look for specific status bar entries that should be present
    // These might include zoom level, image size, pixel values, etc.
    const zoomStatus = await page.locator('.statusbar-item').filter({ hasText: /zoom|Zoom/ });
    if (await zoomStatus.isVisible()) {
      console.log('✅ Zoom status bar entry found');
    }
    
    const sizeStatus = await page.locator('.statusbar-item').filter({ hasText: /size|Size|MB/ });
    if (await sizeStatus.isVisible()) {
      console.log('✅ Size status bar entry found');
    }
