...
async function initWasm(): Promise<any> {
    ...
    wasmInitPromise = (async () => {
        try {
            ...
            const { default: init, decode_tiff, decode_tiff_page, tiff_page_count, extract_exif_tags, demosaic,
                decode_pfm_fast, decode_ppm_fast, decode_npy_fast, decode_fits_fast, decode_netcdf_fast,
                decode_dicom_fast } = await import(wasmBindingsPath);

            // Initialize the WASM module
            await init();

            // The six decode_*_fast entries below back the main-thread
            // decode path in `main-thread-decode.ts`, used when the decode
            // worker is unavailable. They share this one cached module
            // instance rather than initializing a second copy.
            wasmModule = { decode_tiff, decode_tiff_page, tiff_page_count, extract_exif_tags, demosaic,
                decode_pfm_fast, decode_ppm_fast, decode_npy_fast, decode_fits_fast, decode_netcdf_fast,
                decode_dicom_fast };
            return wasmModule;
        } catch (error) {
