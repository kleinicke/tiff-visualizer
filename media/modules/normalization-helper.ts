...
export class NormalizationHelper {
     ...
     * @param settings
     */
    static needsStats(settings: ImageSettings): boolean {
     ...
     * @param normalized - Value in range 0-1
     * @param settings - Settings object with gamma and brightness
     * @returns Corrected value (may be outside [0,1])
     */
    static applyGammaAndBrightness(normalized: number, settings: ImageSettings): number {
     ...
     * @param maxValue - Maximum input value (255 or 65535)
     * @param normMin - Normalization minimum
     * @param normMax - Normalization maximum
     * @returns LUT array mapping input values to output 0-255
     */
    static generateLut(settings: ImageSettings, bitDepth: number, maxValue: number, normMin: number, normMax: number): Uint8Array {
     ...
     * @param settings - Settings object
     * @returns True if identity transformation
     */
    static isIdentityTransformation(settings: ImageSettings): boolean {
        const gammaIn = settings.gamma?.in ?? 1.0;
        const gammaOut = settings.gamma?.out ?? 1.0;
        const exposureStops = settings.brightness?.offset ?? 0;
     ...
     * @param normMin - Normalization minimum
     * @param normMax - Normalization maximum
     * @returns Effective input range
     */
    static getEffectiveVisualizationRange(settings: ImageSettings, normMin: number, normMax: number): { min: number, max: number } {
        const gammaIn = settings.gamma?.in ?? 1.0;
        const gammaOut = settings.gamma?.out ?? 1.0;
        const exposureStops = settings.brightness?.offset ?? 0;
        const range = normMax - normMin;
...
export class ImageStatsCalculator {
     ...
     * @returns Statistics
     */
    static calculateIntegerStats(data: Uint8Array | Uint8ClampedArray | Uint16Array, width: number, height: number, channels: number, rgbAs24Bit = false): { min: number, max: number } {
        let minVal = Infinity;
        let maxVal = -Infinity;
     ...
     */
    static calculateExtendedStats(data: ArrayLike<number>, width: number, height: number, channels: number): { min: number, max: number, mean: number, std: number, validCount: number, nonFiniteCount: number, totalCount: number } {
        let minVal = Infinity;
        let maxVal = -Infinity;
        let sum = 0;
        let sumSq = 0;
...
export class ImageRenderer {
     ...
     * @private
     */
    static _finishSingleChannelRenderHistogram(hist: Uint32Array, nanCount: number, valueRange: { min: number, max: number, isFloat: boolean }, original: { min: number, max: number, sum: number, count: number }): any {
        const histG = new Uint32Array(hist);
        const histB = new Uint32Array(hist);
        const histLum = new Uint32Array(hist);
        const calculateBinStats = (h: Uint32Array) => {
     ...
     * @param isFloat - Whether data is floating point (float16 or float32)
     * @param stats - Image statistics
     * @param settings - Normalization/gamma/brightness settings
     ...
     * @param imageData
     */
    static _flipY(imageData: ImageData): ImageData {
     ...
     * @private
     * @param imageData
     * @param data - original single-channel source data
     * @param isFloat - whether source can contain NaN/Inf
     * @param colormapName
     */
    static _applyDisplayColormap(imageData: ImageData, data: ArrayLike<number>, isFloat: boolean, colormapName: string): void {
