...
import type { RenderOptions, Stats } from './types.js';
...
export class RawProcessor {
    ...
    vscode: VsCodeApi;
    _lastRaw: { width: number; height: number; data: Uint8Array; channels: number; bitDepth: number; maxValue: number } | null;
    _pendingRenderData: boolean | null;
    _isInitialLoad: boolean;
    _cachedStats: Stats | undefined;
    _libRaw: any;
    _workerBootstrapUrl: string | null;
    /** Cached file bytes to avoid re-fetching for full decode */
    _arrayBuffer: ArrayBuffer | null;
    /** Every field from libraw's metadata(), for the Metadata panel */
    ...
    async _withTimeout(promise: Promise<any>, ms: number, label: string): Promise<any> {
    ...
    async _fetchWithTimeout(url: string, ms: number, label: string): Promise<Response> {
        const response = await this._withTimeout(fetch(url), ms, label);
        return response;
    }

    _isTypedArray(value: any): boolean {
        return ArrayBuffer.isView(value) && !(value instanceof DataView);
    }

    _toFinitePositiveInteger(value: any): number | null {
    ...
    _normalizeRawDecodeOutput(rawOutput: any, width: number, height: number, meta: any): { pixelData: Uint8Array; channels: number; payloadShape: string } {
    ...
    async _ensureWorkerBootstrapUrl(): Promise<string> {
    ...
    async ensureWasmLoaded(): Promise<void> {
        if (!this._libRaw) {
            ...
            libRaw.runFn = function(fn: string, ...args: any[]) {
     ...
     */
    _extractEmbeddedJpeg(arrayBuffer: ArrayBuffer): Uint8Array | null {
     ...
     */
    async extractThumbnail(src: string): Promise<{ canvas: HTMLCanvasElement; imageData: ImageData } | null> {
    ...
    /**
     * @param decodeSettings libraw-wasm settings (merged with defaults)
     * @param opts if keepBuffer, don't clear _arrayBuffer after use
     */
    async processRaw(src: string, decodeSettings: object = {}, opts: { keepBuffer?: boolean } = {}): Promise<{ canvas: HTMLCanvasElement; imageData: ImageData }> {
        const loadSignal = this.loadSignal;
        try {
            this._cachedStats = undefined;
            this._lastRaw = null;
    ...
    renderRawWithSettings(): ImageData | null {
