...
export const COLORMAP_NAMES: string[] = [
 ...
 */
function interpolateControlPoints(points: number[][]): number[][] {
	const out = [];
	for (let i = 0; i < 256; i++) {
...
function generateGray(): number[][] {
	const out: number[][] = [];
	for (let i = 0; i < 256; i++) { out.push([i, i, i]); }
	return out;
}

function generateJet(): number[][] {
	const out: number[][] = [];
	for (let i = 0; i < 256; i++) {
		const v = i / 255.0;
 ...
 */
function buildColormap(name: string): number[][] | null {
...
const _inverseCubeCache: Map<string, Uint8Array> = new Map();
 ...
 */
function getInverseCube(name: string): Uint8Array | null {
 ...
 * @returns index 0-255, or -1 if colormap unknown
 */
export function rgbToColormapIndex(name: string, r: number, g: number, b: number): number {
