...
interface RawImageData {
    ...
    isRgbaFormat: boolean;
    /** Whether the image actually has a meaningful alpha channel (native API path only) */
    hasAlpha?: boolean;
    /** Original ImageData from getImageData for zero-copy fast path */
    originalImageData?: ImageData;
}

interface LazyNativeReadback {
...
export class PngProcessor {
    ...
    /** Embedded Exif/text tags, for the Metadata panel */
    _lastAllTags: TagEntry[];
    /** Set by imagePreview.js; called when an async embedded-metadata fetch (JPEG Exif) resolves after the image is already displayed */
    onMetadataTagsReady: (() => void) | null;

    constructor(settingsManager: any, vscode: any) {
     ...
     */
    async processPng(src: string): Promise<{ canvas: HTMLCanvasElement, imageData: ImageData | null, canvasAlreadyRendered?: boolean, lazyPixelData?: boolean, displayElement?: HTMLElement }> {
     ...
     */
    async _loadJpegExifTags(src: string, loadSignal: AbortSignal | undefined): Promise<void> {
        try {
     ...
     */
    _renderToImageData(renderOptions: DeferredRenderOptions = {}): ImageData {
        ...
        if (renderOptions.targetCanvas && this._webglRenderer.canRender({
            data: data as Uint8Array | Uint16Array,
            width,
            height,
            channels,
            isFloat: false,
    ...
    getLazyNativeHistogramImageData(maxPixels = 1_000_000): ImageData | null {
        if (!this._lazyNativeReadback) return null;
    ...
    renderPngWithSettings(renderOptions: DeferredRenderOptions = {}): ImageData | null {
     ...
     */
    _postFormatInfo(width: number, height: number, channels: number, bitDepth: number, formatLabel: string): void {
     ...
     * @param arrayBuffer - PNG file data
     * @returns Bit depth (1, 2, 4, 8, or 16), or null if detection fails
     */
    _detectPngBitDepth(arrayBuffer: ArrayBuffer): number | null {
        try {
