...
interface HistogramData {
	...
	nanCount: number;
	stats: { r: BinStats, g: BinStats, b: BinStats, luminance: BinStats };
}

interface HistogramOptions {
	rawData?: ArrayLike<number> | null;
	planarData?: ArrayLike<number>[] | null;
	channels?: number;
...
interface OriginalStats {
	r: { min: number, max: number, mean: number, total: number };
	g: { min: number, max: number, mean: number, total: number };
	b: { min: number, max: number, mean: number, total: number };
}
...
export class HistogramOverlay {
	 ...
	 * @param isFloat - Whether to format as float
	 * @returns Formatted value
	 */
	formatValue(value: number, isFloat: boolean): string {
		if (isFloat) {
	 ...
	 * @param binIndex - Bin index (0-255)
	 * @returns Original value
	 */
	binToValue(binIndex: number): number {
	 ...
	 * @param binIndex
	 */
	updateTooltip(clientX: number, clientY: number, binIndex: number): void {
	 ...
	 * @param skipNotification - Skip sending visibility notification (used when restoring state)
	 */
	 ...
	 */
	toggleScaleMode(button: HTMLElement): void {
		const modes: ('linear' | 'sqrt')[] = ['linear', 'sqrt'];
	 ...
	 * @param settings - Settings with gamma and brightness
	 * @param inputMax - Maximum input value (255, 65535)
	 * @returns LUT mapping input -> output bin
	 */
	generateTransformLUT(settings: ImageSettings, inputMax: number): Uint8Array {
	 ...
	 * @param settings - Settings with gamma and brightness
	 * @returns LUT mapping quantized float (0-65535) -> output bin (0-255)
	 */
	generateFloatLUT(settings: ImageSettings): Uint8Array {
	 ...
	 *
	 * @param imageData - Canvas ImageData (fallback if no raw data)
	 * @param options
	 */
	computeHistogram(imageData: ImageData | null, options: HistogramOptions = {}): HistogramData | null {
		...
		const safeLut = lut as Uint8Array;
		...
		const calculateBinStats = (hist: Uint32Array) => {
	 ...
	 * @param options
	 */
	update(imageData: ImageData | null, options: HistogramOptions = {}): void {
	 ...
	 * @param precomputed
	 */
	updateFromPrecomputed(precomputed: { histogramData: HistogramData, originalStats?: any, valueRange?: { min: number, max: number, isFloat: boolean } }): void {
	 ...
	 * @param mode
	 */
	setScaleMode(mode: 'linear' | 'sqrt'): void {
